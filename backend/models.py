...
class ActionType(str, Enum):
    ...
    PRIORITIZE_SHIPMENT = "prioritize_shipment"  # Mark shipment as high-priority (costs 1 step)
    HOLD_SHIPMENT       = "hold_shipment"        # Hold a shipment at current node
    CANCEL_ROUTE        = "cancel_route"         # Undo a routing decision (if shipment not dispatched)
...
class ShipmentSummary(BaseModel):
    ...
    deadline_step: int = Field(..., ge=0, description="Absolute time step by which delivery earns full reward")
...
class Action(BaseModel):
    ...
    def rationale_not_boilerplate(cls, v: str) -> str:
        ...
        if v.strip().lower() in blocked:
            raise ValueError("Rationale must be a genuine explanation, not a placeholder.")
        return v.strip()
...
class Observation(BaseModel):
    ...
    # --- Temporal context ---
    current_step: int = Field(..., ge=0, description="Current discrete time step (0-indexed)")
    ...
    recently_read_alert: Optional[UnstructuredAlert] = Field(
        default=None,
        description="Populated only after a READ_INBOX action."
    )
    ...
    warehouse_status: List[WarehouseSummary] = Field(
        default_factory=list,
        description="Current snapshot of all warehouses."
    )
...
class RewardInfo(BaseModel):
    ...
    late_delivery_penalty: float = Field(default=0.0, le=0.0,
        description="Always ≤ 0. Magnitude scales with steps_late × cargo_value.")
    capacity_violation_penalty: float = Field(default=0.0, le=0.0,
        description="Agent attempted to route cargo to a full warehouse.")
    refrigeration_violation_penalty: float = Field(default=0.0, le=0.0,
        description="Refrigerated cargo sent to non-cold warehouse.")
    hazmat_violation_penalty: float = Field(default=0.0, le=0.0,
        description="Hazmat cargo sent with non-certified truck.")
    strike_ignored_penalty: float = Field(default=0.0, le=0.0,
        description="Agent dispatched to a warehouse with active strike.")
    inefficiency_penalty: float = Field(default=0.0, le=0.0,
        description="Repeated no-ops, redundant reads, or dispatching empty trucks.")
    rationale_quality_bonus: float = Field(default=0.0, ge=0.0, le=0.05,
